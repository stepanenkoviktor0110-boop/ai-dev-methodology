...
- **Любой промт для новой сессии (next-session / hand-off / «промт для копирования») ПЕРЕД показом владельцу прогонять через executor-калибровку:** прочитать промт *как исполнитель, которому по нему завтра кодить*, и ответить себе коротко на три вопроса — (1) что уже работает хорошо; (2) чего не хватает для эффективной работы (недостающие контракты, сигнатуры, поля, нерешённые развилки, на которых исполнитель застрянет); (3) что лишнее / сбивает / дублируется / противоречит само себе / ограничивает без смысла. Находки из (2)–(3) **свернуть внутрь промта сразу** (развилки решать автономно по простоте), показывать уже откалиброванную версию + короткий итог проверки. Триггер — сам факт сборки hand-off-промта, не отдельная команда. Это калибровка СТРУКТУРЫ/ПОЛНОТЫ инструкции-агенту — отдельно от `prompt-master`/`prompt-reviewer` (те про LLM-промты приложения) и от рантайм-калибровки ответов модели; не путать.
