...
Перед deploy/SSH/restart → verify: порты (`ss -tlnp`), push state (`git log origin..HEAD`), SSH доступ (вручную), процессы на порту (netstat). Серверные проблемы имеют физические причины (порт занят, ISP-блок, hung process) — проверяй их до дебага логики. ERR_TIMED_OUT с нескольких сетей = ISP-блок → туннель или смена IP. В deploy script — добавлять `pkill -f <service>` перед systemctl restart, чтобы orphan-процессы не блокировали порт при автоматическом деплое.
