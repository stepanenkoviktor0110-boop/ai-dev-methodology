...
- When промт и task file содержат разные значения для одного enum/константы → читать task file как authoritative, зафиксировать deviation до реализации, to не переписывать реализацию после обнаружения конфликта на ревью.
- When pipeline завершается успешно но внешний экспорт падает с auth-ошибкой → добавить алерт на N подряд exported=0 независимо от типа ошибки, to обнаружить молчаливый сбой доставки до жалобы пользователя.
- When файл изменён вне task-плана фичи (hotfix, дополнение вне scope) → явно добавить файл в file lists следующей audit wave, to предотвратить ship неаудированного кода.
- When systemctl restart ssh выполнен, но Connection closed продолжается → убить зависшие клиентские процессы (`pkill -f "sshd: user@"`) + restart, to реально освободить MaxStartups-слоты.
- When пользователь отменяет задачу формулировкой «не нужен» без уточнения границ → уточнить: отменяется UI-слой или вся функциональность (API, DB), to не удалить инфраструктуру которая понадобится при передумывании.
...
- When task-creator ставит depends_on=[N] задаче в той же wave что и задача N → явно проверять wave(зависимости) < wave(задачи) для каждой пары в Phase 0, to предотвратить противоречие «параллельная волна + зависимость от соседней задачи».
- When deploy-волна начинается с git pull на VPS → перед SSH на сервер выполнить `git log origin/branch..HEAD` локально, to не получить блокер «нечего тянуть» из-за незапушенных коммитов.
- When новый endpoint получает только resource_id без user_id → явно добавить проверку роли target-пользователя ресурса относительно прав вызывающего, to предотвратить IDOR через косвенный доступ к данным другого пользователя.
- When запрос «добавить фильтр/сортировку ко всем колонкам» → составить явную таблицу тип-колонки → поведение-фильтра до написания кода, to не строить неполную реализацию которую отклонят.
- When ERR_TIMED_OUT (не ERR_CONNECTION_REFUSED) с нескольких независимых сетей при открытом порту локально → не дебажить server-side — использовать туннель или сменить IP, to не тратить время на дебаг там где проблема на уровне ISP.
