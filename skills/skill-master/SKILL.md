...
- When writing a multi-phase SKILL.md -> before the first skill-checker run: (1) verify each phase ends with a Checkpoint block (final phases and alternative branches like Update Mode are most often missed); (2) verify each self-verification checklist item directly corresponds to a procedure step (same action, same condition) — self-verification written separately from the procedure tends to drift, and the mismatch is only caught in code audit
- When a SKILL.md contains an inline LLM prompt as part of a phase -> check the entire prompt text for negative framing (prohibitions, caps, STRICTLY FORBIDDEN) and rewrite positively before first skill-checker run, to avoid a predictable review finding inside a nested prompt (triad #135)
