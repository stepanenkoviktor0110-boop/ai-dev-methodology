...
**String-output assertions — ограничивай scope (Seen: 2):** Для тестов структурированного string-output (markdown, отчёты) недостаточно проверять наличие значения — извлекай нужную секцию (regex/split), затем проверяй значение внутри неё. Отличает "значение есть" от "значение в нужном месте".
