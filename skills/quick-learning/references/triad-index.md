...
| 95 | PROMOTED → task-decomposition | wave ordering: последовательные задачи внутри одной "названной волны" | — | — | 2 | — | task-decomposition |
...
| 115 | клиент присылает правки к approved user-spec | точечное мини-интервью только по изменённым пунктам, обновить spec напрямую | не запускать полный интервью-цикл заново | situational | 1 | Situational | — |
| 116 | уточнение UI-расположения элемента в spec-интервью | выбрать стандартный UX паттерн без вопроса | не блокировать интервью на деталях которые пользователь не формулирует | situational | 1 | Situational | — |
| 117 | обновление user-spec к уже реализованной фиче | запустить code-researcher для diff новых требований против существующего кода | не создавать задачи для уже реализованного функционала | situational | 1 | Situational | — |
| 118 | task-creator пишет hints для файла, который уже реализован в кодовой базе | прочитать фактический код файла ДО написания hints | не создавать hints противоречащие существующей реализации | universal | 1 | Universal | — |
| 119 | TDD Anchor задачи A называет тестовый файл, являющийся primary deliverable задачи B | убрать тест из TDD Anchor A, добавить ссылку на задачу-владельца | один файл — один владелец, нет конфликта владения при параллельном выполнении | situational | 1 | Situational | — |
| 120 | конфигурационная опция библиотеки молча игнорируется в новой major-версии | проверить поведение в config-file vs CLI через changelog/issues для текущей версии ДО написания конфига | не тратить fix-раунд на конфигурацию которую версия игнорирует | universal | 1 | Universal | — |
| 121 | интеграционный тест проверяет duplicate/error flow в auth-библиотеке | проверять через DB side effect, не HTTP статус | не получить false-negative когда библиотека возвращает 200 с resend flow | universal | 1 | Universal | — |
| 122 | E2E global-setup требует seeded verified user | прямой INSERT с hashPassword, не sign-up API + SQL UPDATE | устранить зависимость seed-фазы от работающего сервера | situational | 1 | Situational | — |
| 123 | добавление нового параметра к существующему API-запросу | проверить nullable поля ответа при новом параметре на реальных данных | не получить TypeError на production-прогоне | universal | 1 | Universal | — |
| 124 | выполнение нетривиального Python-скрипта на удалённом сервере через SSH | записать в локальный файл, залить по SFTP, выполнить python3 -u | избежать ошибок экранирования и буферизации вывода | situational | 1 | Situational | — |
| 125 | запрос "удалить X из репозитория" для локального git-репо | уточнить: git untrack (git rm --cached + .gitignore) или физическое удаление с диска | не уничтожить локальные файлы при git-операции | universal | 1 | Universal | — |
