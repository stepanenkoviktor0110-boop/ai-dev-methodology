...
### 2026-04-12 responsive-layout / session 2: Компонент в файловой системе ≠ компонент в scope

**Seen:** 1
**Adapted:** —
**Triad:** планирование рефакторинга/адаптации компонентов по файловой системе → проверить что компонент реально рендерится (не закомментирован в entry point) до включения в scope → не планировать работу над мёртвым кодом
**Context:** Включил Barista в tech-spec для адаптации, т.к. файлы существовали в src/containers/barista/. Оказалось — компонент закомментирован в page.tsx, как и Advantages (который уже был исключён). Пользователь поймал.
**Pattern:** При планировании рефакторинга, который затрагивает набор компонентов — проверяй entry point (root page, App.tsx) на предмет фактического рендеринга каждого компонента. Наличие файлов в файловой системе не означает что код используется.
**Scope:** universal
**Category:** scope-management
