...
### 2026-03-28 mvp-parser / live-test: Телефоны множат строки для ВСЕХ участников, не только ответчиков

**Seen:** 1
**Triad:** множественные контакты у участника дела → разворачивать телефоны в отдельные строки для ВСЕХ сторон (истцы и ответчики) → не делать ложных предположений что логика применяется только к одной стороне
**Context:** Изначально телефоны истцов планировалось собирать в одну ячейку, а ответчиков — по строкам. Заказчик уточнил: "один телефон — одна строка, для всех". Декартово произведение: телефоны истца × телефоны ответчика. Если у обоих по 3 телефона = 9 строк на пару.
**Pattern:** Когда бизнес-логика работает с контактами участников — применять одинаковые правила ко ВСЕМ сторонам (истцы, ответчики, третьи лица). Не предполагать что логика (развёртка телефонов, обогащение контактов) нужна только для одной стороны — уточнять у заказчика.
**Scope:** universal
**Category:** scope-management
