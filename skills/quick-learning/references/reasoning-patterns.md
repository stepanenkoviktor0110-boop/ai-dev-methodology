...
### 2026-05-13 b24u-playbook-kstore / session 2: pretrained-brand leak through context mention

**Seen:** 1
**Adapted:** —
**Cognitive Error:** pretrained-brand leak
**Triad:** brand or product-family name appears in retrieved context without enumerating the exact members available → enforce explicit anti-pattern examples in the grounding rule ("do NOT name <Model-X-from-brand> if not literally in context"), not just a generic "use only the source" rule → assume that "use only the source" is enough to prevent the model from filling in plausible-sounding members of the named brand from pretraining
**Context:** In a retrieval-augmented chat for an electronics shop, the proxy-fed knowledge base contained only accessories for robot vacuums and only one electric scooter model. On user queries like "robot vacuum xiaomi" and "scooter under 50k", the bot fabricated specific Xiaomi and Kugoo SKUs ("Mi Robot Vacuum-Mop 2 Pro", "Kugoo S1 Pro/G2 Pro/G-Max", "Redmi Note 14 Pro"), invented matching prices, and rendered fake product URLs in the catalog pattern of the shop — every link 404. The grounding rule said "names, prices, URLs — only from visible context"; the model ignored it because the brand was mentioned in context (one accessory had "Xiaomi" in the name), and the model treated the brand mention as license to enumerate well-known members of that brand from its pretrained world model.
**Pattern:** When grounding a retrieval-augmented system whose context can contain brand mentions without exhaustive product enumeration, do NOT rely on a generic "only from the source" rule. Add the **anti-pattern examples**: cite concrete model strings that the model is likely to fabricate from pretraining and forbid them explicitly ("do not write `Mi Vacuum 2 Pro` if that string is not in the context"). Add the **accessory→core inference rule**: presence of an accessory for category X is not evidence that X itself is available — bot must say so and escalate, not fill. Add a **link-rendering rule**: never construct URLs from a template (e.g. `/catalog/<slug>/<id>/`), only quote URLs literally shown. Verify by spot-checking 3+ category responses against the actual feed and curl'ing every returned URL — a generic rule that passes one test (iPhone in catalog) can still fail on adjacent gaps (Xiaomi vacuums in accessories-only state).
**Scope:** universal
**Category:** problem-decomposition

### 2026-05-13 b24u-playbook-kstore / session 2: retrieval-test green is not application-green

**Seen:** 1
**Adapted:** —
**Cognitive Error:** test-surface gap
**Triad:** validation framework offers a "retrieval test" that runs the search step on demand AND the end-user surface is a separate cached pipeline → after each prompt/knowledge-base change, always re-test on the user surface (chat, app, live page) with a forced cache reset, not only on the on-demand test surface → confuse "retrieval returns the right snippet now" with "the user gets a correct answer now"
**Context:** Platform's admin "RAG test" tool ran the retrieval immediately, showed real products with real URLs in the context preview. I declared the fix successful and only afterwards ran the demo chat — the chat kept producing hallucinated content from the old configuration for several minutes. The platform server-side caches the assembled context for the production chat with a TTL of minutes; the admin retrieval tool bypasses that cache. The admin tool's green light gave a false signal of "fixed". I had to also call `resetContextCache(client_id)` (a hidden admin function) and start a fresh chat session before the change actually took effect.
