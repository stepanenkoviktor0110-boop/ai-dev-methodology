...
**Seen:** 1
**Adapted:** —
**Triad:** структурное изменение одного элемента из sibling-группы → применить симметрично всем братьям → консистентный UI
**Context:** Переносил hint-текст и убирал cond-строку в одной карточке лояльности — не сделал то же самое в соседней. Пользователь: «почему ты не понимаешь, что все блоки должны быть как братья близнецы?»
**Pattern:** Перед коммитом структурного изменения в повторяющемся UI-элементе — найти все sibling-экземпляры и применить то же изменение к каждому. Одна карточка изменилась → все карточки изменились.
**Scope:** universal
