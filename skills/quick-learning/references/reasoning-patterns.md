...
**Pattern:** При повторном падении делегированной задачи — после первой неудачи проверить ресурсные ограничения среды (RAM, процессы, диск), а не переписывать промт. Если инструкции не помогают с первого раза — проблема вероятно не в инструкциях.
**Scope:** universal
**Category:** tool-selection
...
**Pattern:** Когда проектируешь систему, которую реализует другой исполнитель — до финализации архитектуры явно перечислить инструменты исполнителя. Если архитектура зависит от инструмента, к которому исполнитель не имеет доступа — заложить универсальный интерфейс с самого начала, а не после.
**Scope:** universal
**Category:** tool-selection
...
**Pattern:** Если в схеме данных есть поле-классификатор (тип, вид, категория) — проверить: имеют ли все значения одинаковый набор атрибутов? Если нет (одни значения требуют ИНН/р/с, другие — день рождения/телефон) — это разные сущности, связанные M:M, а не одна таблица с полем "тип".
**Scope:** universal
**Category:** problem-decomposition
...
**Pattern:** Когда стейкхолдер говорит "да" на структуру из N элементов — это подтверждение концепции, не валидация каждого элемента. Особенно если один из элементов основан на незавершённом/закомментированном коде — это сигнал, что элемент требует отдельного обсуждения перед включением в scope.
**Scope:** universal
**Category:** scope-management
...
**Pattern:** При удалении кода, который выглядит самодостаточным, проверить: менял ли он состояние чего-то другого (стили, флаги, видимость). Если да — убедиться, что дефолтное состояние управляемого элемента корректно без управляющего.
**Scope:** universal
**Category:** problem-decomposition
...
**Pattern:** Когда ресурс становится доступным через посредника (кэш, прокси, роутер), проверить: все ли потребители идут через этого посредника? Серверные процессы, фоновые задачи, SSR-запросы часто обходят пользовательский путь и обращаются напрямую.
**Scope:** universal
**Category:** information-gathering
