...
**Seen:** 1
**Adapted:** —
**Triad:** делегирование кодогенерации внешнему AI-агенту на много файлов → инструктировать "пиши файлы по одному, не все разом" → предотвратить зависание агента на генерации гигантского патча
**Context:** Codex дважды завис (14 мин без прогресса) пытаясь сгенерировать ~20 файлов одним ответом. После инструкции "write files ONE AT A TIME" задача завершилась за 5 мин.
**Pattern:** При делегировании кодогенерации внешнему AI-агенту (Codex, Cursor, Copilot Workspace) явно указывай "пиши файлы по одному". Большие батчи вызывают timeout/hang на стороне модели.
**Scope:** universal
**Category:** tool-selection

### 2026-04-07 tree-constructor / session 1: Жёсткий таймаут прогресса для async-агента

**Seen:** 1
**Adapted:** —
**Triad:** мониторинг async AI-агента без нового прогресса → установить порог "5 мин без записи в лог = убить и перезапустить" → не тратить 15+ мин на polling заведомо зависшей задачи
**Context:** Codex-задача не обновляла лог 15 минут, но мы продолжали проверять статус 4 раза вместо быстрого kill+restart. Потратили ~15 мин контекста на бесполезный мониторинг.
**Pattern:** При мониторинге async AI-агента установи жёсткий порог: если лог не обновлялся 5 минут — kill и перезапуск. Не полагайся на "может ещё думает".
**Scope:** universal
**Category:** recovery

### 2026-04-07 freelance-autopilot / mvp: Entry point в плане MVP

**Seen:** 1
**Adapted:** —
**Triad:** план MVP содержит только модули без entry point → до начала кодирования проверить что план включает runnable entry point и npm start → пользователь может запустить и проверить результат без помощи разработчика
**Context:** 12 задач MVP были определены и выполнены, но оркестратор (src/index.js) не был в плане — обнаружилось только когда нетехнический пользователь спросил "как мне проверить?"
**Pattern:** При планировании модульного проекта всегда включай задачу на создание runnable entry point (main script + npm start). Проверяй: "может ли конечный пользователь запустить это одной командой?"
**Scope:** universal
**Category:** scope-management

### 2026-04-07 freelance-autopilot / mvp: Роли AI-агентов до старта

**Seen:** 1
**Adapted:** —
**Triad:** несколько AI-агентов работают над одним проектом → согласовать разделение ролей ДО первой задачи → не корректировать роли на лету с потерей контекста и переделками
**Context:** Codex начал писать и код и тесты, пользователь скорректировал посреди сессии: "Codex — только код, Claude Code — проверки, аудит, деплой". Часть работы пришлось пересогласовать.
**Pattern:** Перед началом multi-agent сессии явно зафиксировать кто за что отвечает (код / ревью / тесты / деплой) и записать в feedback memory. Не начинать первую задачу пока роли не согласованы.
**Scope:** universal
...
**Seen:** 1
**Adapted:** —
**Triad:** анимация переводится с page-load на IntersectionObserver → пересчитать все CSS `animation-delay` под новый момент старта → задержки для page-load (1.6s+) неприемлемы для scroll-trigger: пользователь уже смотрит на блок
**Context:** Иконки имели delay 1.6–3.1s (под исчезнувшую hint-плашку). После подключения IntersectionObserver пользователь видел блок, но анимации не было — казалось, страница зависла.
**Pattern:** При переносе анимации с page-load на scroll-trigger: уменьшить первый delay до 0.2–0.4s. Правило: delay[0] ≤ 0.5s от момента срабатывания observer. Все последующие — каскадом по 0.2–0.3s.
**Scope:** universal
**Category:** sequencing

---

### 2026-04-06 mbc-site / demo: IntersectionObserver threshold для section-анимаций — начинать с 0.1

**Seen:** 1
**Adapted:** —
**Triad:** scroll-triggered анимация на большом блоке → threshold: 0.1 как стартовое значение → threshold: 0.4 требует глубокого скролла внутрь блока, пользователь жалуется «нужно полностью перейти»
**Context:** threshold: 0.4 на секции меню (высота ~500px) = нужно проскроллить ~200px внутрь блока. Пользователь ожидал срабатывания как только блок появился на экране.
**Pattern:** Для анимаций-приветствий секций → threshold: 0.1–0.15. Для lazy-load контента который нужно дочитать → 0.3–0.5. Для sticky/pin эффектов → 0 (любой пиксель).
**Scope:** universal
**Category:** tool-selection

---

### 2026-04-06 mbc-site / demo: SVG-стрелка к динамической цели — JS или отказ от подхода

**Seen:** 1
**Adapted:** —
**Triad:** SVG дуга должна указывать на элемент с позицией зависящей от viewport → предложить JS-позиционирование ИЛИ альтернативный дизайн СРАЗУ → не итерировать пиксельные координаты вручную
**Context:** Пытался 6+ раз подобрать endpoint SVG-пути к первой иконке. Иконки расположены flex-column с clamp-размерами — позиция меняется на каждом viewport. Фиксированный `top: 6%` + фиксированные SVG координаты дают разный результат на разных экранах. Пользователь в итоге полностью отказался от подхода.
**Pattern:** Как только цель SVG-стрелки — flex/grid элемент с динамической позицией: (1) предложить JS getBoundingClientRect для позиционирования, (2) или предложить альтернативный дизайн без точного прицеливания. Ручной перебор координат — тупик при 2-й итерации без результата.
**Scope:** universal
