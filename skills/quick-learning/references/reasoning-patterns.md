...
### 2026-04-05 mbc-site / demo: Fetch first — проверка контента по источнику

**Seen:** 1
**Adapted:** —
**Triad:** запрос «проверь соответствие первоисточнику» → сначала fetch источника, потом правки → не изобретать данные
**Context:** Попросили сверить контент демо с mbcculture.by — вместо этого внёс правки по памяти, изобрёл названия карт и адрес, получил "ты охренел?" и несколько fix-раундов.
**Pattern:** Когда задача содержит слово «проверь» или «сравни с источником» — первым действием fetch источника, только потом любые изменения. Нет fetch = нет доверия к данным.
**Scope:** universal
**Category:** information-gathering

### 2026-04-05 mbc-site / demo: Симметричные изменения для sibling-элементов

**Seen:** 2
**Adapted:** —
**Triad:** изменение/фикс одного элемента из sibling-группы (UI-компонент, DB-запрос, API-эндпоинт) → явно проверить/применить к каждому брату → не оставить соседей в рассинхронизированном состоянии
**Context:** Переносил hint-текст в одной карточке лояльности — не сделал в соседней; исправил role-фильтр в одной вкладке отчёта — другая вкладка сохранила старый фильтр до постдеплойного баг-репорта.
**Pattern:** При изменении/фиксе одного из N параллельных элементов — grep по feature-directory на тот же паттерн и явно проверить каждый sibling. Применимо к UI-компонентам, DB WHERE-фильтрам, route-guards — любая группа братьев дрейфует молча.
**Scope:** universal
...
### 2026-04-05 mbc-culture-demo / session 3: бренд-стек клиента приоритетнее референс-шрифтов

**Seen:** 1
**Adapted:** —
**Triad:** редизайн существующего бренда по референс-сайтам → сначала спарсить реальные шрифты и цвета живого сайта клиента, потом брать из референсов → не навязывать клиенту чужой бренд-стек
**Context:** Выбрал Playfair Display из coffeemania.ru-референса; реальный сайт клиента использует Montserrat Alternates — обнаружено только после вопроса клиента про шрифты.
**Pattern:** При редизайне с существующим брендом — первым шагом парсить живой сайт клиента (CSS variables, font-face, og:image) и зафиксировать реальную палитру и шрифты. Только после этого обращаться к референс-сайтам для вдохновения. Бренд-стек клиента — source of truth, референс — направление стиля.
**Scope:** situational
**Situation:** задача "редизайн" или "сделать как X, но для бренда Y"
...
### 2026-04-05 mbc-culture-demo / session 3: один термин — несколько реализаций на сайте

**Seen:** 1
**Adapted:** —
**Triad:** задача называет раздел одним словом, на целевом сайте несколько вариантов под этим словом → показать найденные варианты и спросить какой нужен до реализации → не переделывать уже написанный блок
**Context:** Задача "добавь Subscription" — на mbcculture.by есть подписка на зерно И программа лояльности. Реализовал первый вариант, получил ТЗ на второй, переписал полностью.
**Pattern:** Если задача содержит термин, под которым на целевом сайте/продукте сосуществуют два разных раздела — вывести оба варианта одной фразой и ждать выбор до начала кодирования. Стоимость вопроса = 0; стоимость переделки = целая сессия.
**Scope:** situational
**Situation:** добавление раздела в редизайн существующего сайта, термин в задаче многозначен на сайте клиента
**Category:** information-gathering
...
### 2026-04-04 juridical-parser / migration: Данные пропали после деплоя — проверить write-routing

**Seen:** 1
**Adapted:** —
**Triad:** данные пропали из ожидаемого хранилища после планового деплоя → проверить git log деплоя — искать коммиты меняющие логику write-routing (куда пишется, не что пишется) → не тратить время на диагностику данных когда проблема в маршрутизации записи
...
### 2026-04-04 methodology-sync / session 1: git -C при работе с несколькими репо

**Seen:** 1
**Adapted:** —
**Triad:** несколько git репо в одной bash сессии → всегда указывать `git -C /path/repo` вместо надежды на сохранённую рабочую директорию → избежать silent failures от команд выполненных из неправильного репо
**Context:** При синхронизации локальных скиллов с публичным репо несколько git команд выполнились молча из неправильной директории — вывод был пустым, ошибки не было.
**Pattern:** При работе с несколькими git репо в одной сессии — всегда указывать репо явно через `git -C /path/repo`. Никогда не полагаться на рабочую директорию после `cd` — в следующем bash-вызове она может быть другой.
**Scope:** universal
**Category:** tool-selection

### 2026-04-04 methodology-sync / session 1: обновлять index-файлы после синхронизации содержимого

**Seen:** 1
**Adapted:** —
**Triad:** синхронизация набора файлов или папок → после синхронизации проверить все index-файлы (README, MEMORY.md, CLAUDE.md) которые описывают синхронизированное содержимое → не оставить устаревший индекс с неверным составом
**Context:** После синхронизации скиллов README описывал старый состав — пользователь уточнил отдельным вопросом.
...
### 2026-04-04 mbc-culture / session 1: запросить референс до вёрстки демо

**Seen:** 1
**Adapted:** —
**Triad:** клиент просит визуальное демо без конкретных референсов → запросить 1-2 сайта-референса + "что раздражает в текущем" ДО начала вёрстки → сократить итерации с 3+ до 1-2
**Context:** Дизайн-демо для клиентского сайта итерировался 3 раза (слишком детское → острые углы → сплит-герой), потому что референс (Coffee&Co скриншот) появился только на 3-м раунде.
**Pattern:** Перед вёрсткой любого дизайн-демо для клиента задать два вопроса: "покажи 2-3 сайта которые нравятся" и "что конкретно раздражает в текущем". Референс всегда сократит итерации.
**Scope:** situational
**Situation:** Работа над визуальным демо/прототипом для клиента, у которого нет чёткого ТЗ на визуал.
**Category:** design-iteration

### 2026-04-04 mbc-culture / session 1: делегировать скан внешнего репо subagent'у

**Seen:** 1
**Adapted:** —
**Triad:** нужно изучить внешнее репо (GitHub) или прочитать >5 файлов подряд в главной сессии → делегировать сканирование Explore subagent'у одним вызовом → не исчерпать контекст главной сессии
**Context:** Предыдущая сессия по тому же проекту упала (стала неотзывчивой) после серии `gh api` bash-вызовов + чтения файлов + попыток загрузить большие PDF/изображения. Пришлось перезапускать и вставлять всё содержание сессии вручную.
...
### 2026-04-05 demo-mbc / session 1: «убери отсюда» ≠ «убери везде»

**Seen:** 1
**Adapted:** —
**Triad:** пользователь говорит "X должно быть [здесь], а не [там]" → добавить в новое место, убрать только из названного неверного, остальные вхождения оставить → не потерять контент трактуя "убери отсюда" как "убери из всех мест"
**Context:** Название напитка отображалось в двух местах (оверлей на фото + правая панель). Пользователь сказал «должно быть поверх, а не в меню». Я удалил из обоих мест — потребовалось три fix-раунда на восстановление. Пользователь имел в виду: убрать с фото, оставить в панели.
**Pattern:** Когда пользователь говорит «X должно быть [здесь], а не [там]» — «там» это конкретное упомянутое место, не все вхождения. Сначала добавь в новое место, убедись что работает, затем удали только из явно указанного неверного.
**Scope:** universal
**Category:** scope-management

### 2026-04-05 demo-mbc / session 1: элемент на стыке тёмного и светлого фона требует сплошного фона

**Seen:** 1
**Adapted:** —
**Triad:** UI-элемент позиционируется на границе между тёмным фото и светлым контентным блоком → использовать сплошной непрозрачный фон (не rgba с alpha < 0.5) → элемент читается на обоих фонах без дополнительных CSS-вариаций
**Context:** Иконки с rgba(184,115,51,0.18) хорошо смотрелись на тёмном фото но стали почти невидимы на кремовой info-панели когда strip переехал на стык колонок. Пришлось переделать на solid var(--espresso).
**Pattern:** Элемент на стыке двух контрастных фонов обязан иметь сплошной непрозрачный фон. Полупрозрачный фон всегда работает только для одной стороны. Определи фон до позиционирования на стык.
**Scope:** situational
**Situation:** CSS-элемент перекрывает одновременно тёмный и светлый блок (стык колонок, оверлей на фото с переходом в белый).
**Category:** design-taste

### 2026-04-05 employee-cabinet / session 1: error boundary перед отладкой краша

**Seen:** 1
**Adapted:** —
**Triad:** runtime краш на деплое/тесте даёт только "Application error" без деталей → добавить error boundary (Next.js error.tsx или аналог) перед итерациями отладки → получить точный стек ошибки с первой попытки вместо слепых fix-раундов
**Context:** /admin/reports крашился при переключении вкладок. Без error.tsx потребовалось 3 деплоя вслепую. После добавления error.tsx ошибка "Cannot read properties of undefined (reading 'Р')" указала на причину немедленно.
**Pattern:** Перед любой отладкой runtime-краша на удалённом окружении — сначала добавить видимый error boundary (error.tsx, ErrorBoundary, Sentry breadcrumbs). Без него каждая итерация — это слепой выстрел. Один деплой с error boundary экономит N итераций исправлений.
**Scope:** universal
**Category:** recovery

### 2026-04-05 employee-cabinet / session 1: React typed state race при смене режима

**Seen:** 1
**Adapted:** —
**Triad:** React компонент хранит typed async данные (массив с определённой формой), UI переключает режим/таб → сбросить данные в [] немедленно при смене режима + добавить type guard перед рендером типизированных данных → предотвратить TypeError из-за промежуточного рендера со stale данными
**Context:** При переключении с вкладки "personal" на "timesheet" React рендерил с tab='timesheet' и loading=false до того как useEffect сбросил data. data содержала PersonalRow[] без поля counts, код обращался emp.counts['Р'] → TypeError.
**Pattern:** Если разные режимы UI ожидают данные разной формы (shape) — всегда сбрасывать data в пустое состояние в начале эффекта и проверять type guard перед рендером типизированных полей. React рендерит синхронно при смене состояния, useEffect запускается асинхронно после.
**Scope:** situational
