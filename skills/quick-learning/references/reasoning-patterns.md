...
### 2026-05-08 rebuild-site-no-tilda / session -02c+-03: handing off a URL after process restart without verifying which process serves it

**Seen:** 1
**Adapted:** —
**Cognitive Error:** default-address hand-off assumption
**Triad:** after restarting a long-running local process and reporting an address (URL/port/path) for the user to verify against → before naming the address, probe a marker that exists only in the freshly-restarted code path and confirm the live response carries it; if a previous instance is still bound, name the actual new address instead of the canonical default → orphan-binding handoff: assuming the canonical default address points at the new process when a stale orphan from a previous run can hold the same address and silently serve outdated behaviour
**Context:** After stopping the prior dev process via the supervising harness and starting a new one, the new instance bound to a non-default port (3002) because an orphan from the earlier session was still occupying the canonical default (3000). The orphan's compiled cache predated the latest code, so it answered HTTP 200 to a smoke curl while serving an out-of-date bundle without the new feature. Reporting the canonical-default URLs to the user looked correct — the smoke check passed — but the user opened them, hit the orphan, and saw "ничего не открывается". Reverse signal: a curl that returns 200 only proves "something is listening on this port", not "the process I just started is listening on this port". The startup log of the new process was the source of truth for which address it actually claimed.
**Pattern:** before reporting any address back to the user after a restart, derive the address from the new process's own startup log (or by probing a marker the new code emits but the old one cannot — a new identifier in HTML, a new route, a new header), not from convention. When the canonical default is taken by a leftover process and the platform forbids killing it, hand off the alternate address explicitly with a one-line note about why it differs. Apply whenever a previous instance might survive a stop signal: Windows node processes after task-killer wraps, Docker containers across reload, daemons that ignore SIGTERM, or any "stop the old / start the new" pattern where the old can outlive the stop.
**Scope:** universal
**Category:** communication

### 2026-05-08 rebuild-site-no-tilda / session -03: E2E test loop reusing one runtime context across cases

**Seen:** 1
**Adapted:** —
**Cognitive Error:** test isolation assumed by default
**Triad:** writing a verification loop that iterates multiple cases through one long-lived runtime instance (browser context, page, DB connection, in-memory state) → create a fresh runtime instance per iteration whenever the act-under-test mutates instance-level state (cookies, DOM, focus, route history, hash) → cross-case state leakage: assuming each iteration starts from a clean baseline when the runtime carries over residue (open dialogs, cached subscriptions, stale event listeners, surviving hash) from the previous case
**Context:** A Playwright verification script iterated ten popup deeplinks through a single page object. The first case opened the popup, the close-step set `location.hash=''` correctly, but on the next `page.goto` to a new hash the handler's internal React state and prior subscription survived, so subsequent cases either saw no dialog or saw a dialog with the wrong aria-label (stale from the previous case). The script reported 3/10 PASS, including a misleading PASS where the iframe count happened to be >0 from the earlier popup. Switching to a fresh `browser.newContext()` per iteration — and increasing the post-navigation wait to let React mount — produced 10/10. The cognitive error was treating a single page across iterations as equivalent to ten independent runs because URLs differed; in reality, all instance-level state (subscriptions, history stack, body styles, residual DOM nodes from in-flight unmounts) carries over.
**Pattern:** when a verification loop's cases each mutate instance-level state of the runtime, default to one fresh runtime instance per case rather than reusing one. The cost is a few hundred ms per case; the saving is not chasing flaky failures and not trusting false positives. The signal that reuse is unsafe: the act-under-test changes anything stored on the runtime that outlives a single case (route, focus, cookies, observers, body classes, DOM siblings outside the test's read scope). Reuse is safe only when each case is purely additive read-only against shared state.
**Scope:** universal
**Category:** tool-selection
