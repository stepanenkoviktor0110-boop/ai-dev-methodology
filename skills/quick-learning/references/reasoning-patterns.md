...
### 2026-03-30 juridical-parser / deploy: Метрика в статус-дашборде — за запуск, не за всю историю

**Seen:** 1
**Triad:** добавление счётчика в status/dashboard → считать за текущий запуск (из pipeline summary), не агрегировать по всей БД → пользователь видит что происходит сегодня, а не накопленный итог
**Context:** Реализовал "ответчиков с/без телефона" как SQL-запрос по всей БД. Пользователь поправил: нужно за текущий запуск. Накопленные totals бесполезны для оценки качества конкретного прогона.
**Pattern:** Когда добавляешь метрику в статус-вкладку/дашборд — уточни: за текущий запуск или за всю историю? По умолчанию операционные метрики (обогащено, найдено, экспортировано) — за запуск. Исторические (всего в БД, квота за месяц) — агрегат. Не смешивай.
**Scope:** universal
**Category:** scope-management
