...
8. AC для markdown-only фич: через наличие конкретных артефактов, не описаний (Seen: 1)
9. Client state (useState) в server component → явно предписать client wrapper pattern, запретить "use client" на layout (Seen: 1)
10. Задача на удаление — проверить dead variables, stale comments, duplicate tests до ревью (Seen: 1)
